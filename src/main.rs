...
fn list_devices(verbose: bool) -> rusb::Result<()> {
...
fn print_device(device_desc: &rusb::DeviceDescriptor, handle: &mut Option<UsbDevice>) {
...
fn print_config(config_desc: &rusb::ConfigDescriptor, handle: &mut Option<UsbDevice>) {
...
fn print_interface(interface_desc: &rusb::InterfaceDescriptor, handle: &mut Option<UsbDevice>) {
...
fn print_endpoint(endpoint_desc: &rusb::EndpointDescriptor) {
...
fn get_speed(speed: rusb::Speed) -> &'static str {
    match speed {
        rusb::Speed::Super   => "5000 Mbps",
